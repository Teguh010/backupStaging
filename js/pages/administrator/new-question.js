...
function getAnswerType(question_type_id) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (question_type_id == 1 || question_type_id == 8) {
                $('.form-question-' + no + ' .panel-answer-type').show();
                $('.form-question-' + no + ' .mcq_input_answers_div').show();
                if (question_type_id == 1) {
                    $('.form-question-' + no + ' .panel_answer_type').html(`<button class="btn btn-square btn-outline-info font400 mr-1 mb-2 btnAnswerType" data-type="` + question_type_id + `" data-id="1">Single Correct Answer</button>`);
                    $('.form-question-' + no + ' .correct_answer_multiple').hide();
                    $('.form-question-' + no + ' .correct_answer_single').show();
                    ...
                    <div class="col-lg-12 mb-2">
                        <div class="form-inline">
                            <div class="form-group" style="width: 100%;text-align: center;">
                                ...
                                <a style="cursor: pointer; text-decoration: none;"
                                    class="removeInstructionContent text-danger-active fs18 ml-2" data-index="`+ instruction_content_id + `" data-id="image"><i
                                        class="fa fa-times"></i></a>
                            </div>
                        </div>							
                    </div>
...
function addArticleContent(type) {
    ...
    if (type == 'text') {
        ...
        var content = `
                <div class="panel_input_article">
                    <div class="col-lg-11 mb-2">
                        <span id="article_text_` + article_content_id + `" style="min-height: 50px; height: auto; text-align:left;" class="form-control math_text"></span>
        ...
        var content = `
                <div class="panel_input_article">
                    ...
                    <div class="col-lg-1 mb-2">                        
                        <a style="cursor: pointer; text-decoration: none;"
                                    class="removeArticleContent text-danger-active fs18 ml-2" data-index="`+ article_content_id + `" data-id="image"><i
                                        class="fa fa-times"></i></a>
                    </div>
                </div>
        `;
...
function addQuestionContent(type) {
    ...
    if (type == 'text') {
        ...
        var content = `
                <div class="panel_question_content_` + no + `_` + question_content_id + `">
                    <div class="col-lg-11 mb-2">                    
                        <span id="question_text_`+ no + `_` + question_content_id + `" style="min-height: 50px; height: auto; text-align:left;" class="form-control math_text math_text_` + question_content_id + `"></span>
...
function create_question() {
    ...
    $.ajax({
        type: 'POST',
        url: base_url + 'administrator/create_question',
        ...
        success: function (res) {
            ...
            if (res.msg == 'success') {
                ...
                swal({
                    ...
                    .then((OK) => {
                        if (OK) {
                            window.location.replace(base_url + 'administrator/questions');
                        }
                    });
            }
        }
