							...
							<table class="table worksheet_table datatable">
								...
								<?php
									foreach ($user as $users) {
										if (count($users) == 0) {
											echo '<div class="tutor-row clearfix">';
												echo '<div class="col-xs-offset-1 col-sm-offset-1 col-md-offset-1 col-lg-offset-1 col-xs-10 col-sm-10 col-md-10 col-lg-10">';
													echo '<div class="alert alert-danger margin-top-custom text-center">Don\'t have any tutor in the listing.</div>';
												echo '</div>';
											echo '</div>';
										} else {
											$i = 1;
											foreach($users as $temp_user){
												...
												echo '<td id="td-tutor-'.$temp_user->user_id.'">';
												$check_tag = $this->model_admin->check_class_tag($class_id, $temp_user->user_id);
												if($check_tag==0)
													echo '<a id="tutor_'.$temp_user->user_id.'_'.$class_id.'" class="btn btn-custom btn-no-margin-top assign_class_btn" title="Tag Tutor to Class"><i class="fa fa-user"></i></a>
														';
												else
													echo '<a class="btn btn-primary btn-no-margin-top" title="Tagged"><i class="fa fa-check"></i></a> <a class="btn btn-danger btn-no-margin-top" title="Remove Tag" onclick=\'removeTagTutor("'.$temp_user->user_id.'","'.$class_id.'")\'><i class="fa fa-trash"></i></a>';
												echo '</td></tr>';
												$i++;
											}
										}
...
<script type="text/javascript">
	...
	function removeTagTutor(tutorId,classId) {
		$.ajax({
			url:'<?php echo base_url()?>administrator/removeTagTutor',
			method: 'post',
			data: {
				class_id: classId,
				tutor_id: tutorId,
			...
			success: function(response){
				$('#td-tutor-'+tutorId).html('');
				var html = '<a id="tutor_'+tutorId+'_'+classId+'" class="btn btn-custom btn-no-margin-top assign_class_btn" title="Tag Tutor to Class"><i class="fa fa-user"></i></a>';
				$('#td-tutor-'+tutorId).append(html);
			}
		});
	}
	...
	$(document).ready(function(){
		...
		$(document).on('click', '.assign_class_btn', function(e) {
			...
			var classId = tutorIdArray[2];
			$.ajax({
				url:'<?php echo base_url()?>administrator/tagStudent/tutor',
				method: 'post',
				data: {
					class_id: classId,
					student_id: tutorId,
				...
				success: function(response){
					...
					var html = `
							...
							<a class="btn btn-danger btn-no-margin-top" title="Remove Tag" onclick="removeTagTutor(`+ tutorId +`,` + classId + `)" ><i class="fa fa-trash"></i></a>
						`;
					$('#td-tutor-'+tutorId).append(html);
				}
			});
