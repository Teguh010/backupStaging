							...
							foreach ($questionList AS $question) {
								...
								if(isset($is_admin) && $is_admin == FALSE) {
									...
									if($num_of_question> 1){
										$close_btn = '<a href="#" class="question-remove" title="Remove Question" data-id = "'.$question->question_id.'"><i class="fa fa-times"></i></a>';
									}
								...
								if($this->session->userdata('worksheetSubject') == 7) {
									...
									if(empty($strategyList[$quesNum-1])){
										echo '<div class="question_number"> '. $dropdown .' 
										...
										echo '<div class="question_number_with_strategy"> '. $dropdown .' 
											<span class="pull-right question_category">[' . $substrandList[$quesNum - 1] . '] ' . $categoryList[$quesNum-1].$close_btn.'</span>
											<br>
											<span class="question_strategy">'.$strategyList[$quesNum-1].'</span>
											<br>
											<span class="pull-left question_difficulty_with_padding">('.$question->difficulty.' Marks, '.$difficulty.')</span>
										</div>';
									}
								}
								...
								echo nl2br($question->question_text);
								...
								if(isset($is_admin) && $is_admin == FALSE) {
									if($subquestionList[$quesNum - 1] === 'B'){
										echo '<button class="btn btn-warning pull-right sub_question" id="subqid_'.$question->question_id.'" data-id=""><span data-toggle="tooltip" data-placement="top" title="Add sub question, please">Sub Question</span></button>';
										echo '</li>';
										echo '</div>';
									}else {
										echo '</li>';
...
<div class="modal fade" id="addMoreModal" role="dialog">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="question_number add_more">
				...
				<div class="pull-left add_more_sub_top">
					<span class="label label-warning">
						<?php echo $worksheet_substr . " / " . $worksheet_topic;?>
					</span>
				</div>
				<div class="pull-left add_more_str">
					<span class="label label-warning">
						<?php echo $worksheet_strategy;?>
					</span>
				</div>
			</div>
			...
			<div class="modal-footer">
				...
				<input type="button" class="btn btn-default" data-dismiss="modal" value="Close">
				<a class="btn btn-custom" href="<?php echo base_url();?>smartgen/submit_add_more" id="submit_addmore_btn">Add Questions</a>
			</div>
			</form>
		</div>
	</div>
...
$(document).ready(function (){
	...
	// setTimeout(append_div_mathjax, 1000);A
	...
	$(function(){
		$('#addMoreModal').on('shown.bs.modal', function (e) {		
			ajax_more_questions(1);			
		});
	
		$("body").on('click','#pag-addMore a', function(e){		
			e.preventDefault();			
// 			console.log($(this).data("ci-pagination-page"));			
			ajax_more_questions($(this).data("ci-pagination-page"));
		});
		
		$("body").on('click','.grid .grid-item', function(e){			
			e.preventDefault();
			//console.log('here');
			$(this).toggleClass('selected');		
			update_selected_questions($(this));
		});
	});
	
	function update_selected_questions(obj){	
		$.ajax({
			url: "<?php echo base_url(); ?>smartgen/ajax_selected_question",
			type: 'POST',
			dataType: 'text', // added data type
			data: {question_id:obj.data("id")},
			success: function(res) {
				//console.log(res);
			}
		});		
	...
	function ajax_more_questions(page){			
		...
		$.ajax({
			url: "<?php echo base_url(); ?>smartgen/get_more_questions/"+page,
			type: 'POST',
			dataType: 'text', // added data type
			data: {regenerateWorksheet:true},
			success: function(res) {
