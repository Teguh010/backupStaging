...
$(document).ready(function () {
    ...
    $(document).on('click', '.regen_all', function () {
        $.ajax({
            url: base_url + 'ExamMode/regenerateAllQuestion',
            type: 'POST',
            dataType: 'json',
            success: function (res) {
                if (res.msg == 'success') {
    ...
    $(document).on('click', '.regen_question', function (e) {
        ...
        $.ajax({
            url: base_url + 'ExamMode/regenerateQuestionID/' + id + '/' + que_type,
            method: 'POST',
            dataType: 'json',
            success: function (data) {
                ...
                $('#question_' + quesNum + ' .question_difficulty').animate({ opacity: 0 }, function () {
                    ...
                    $(this).append(`(` + data['question'].difficulty + ` Marks)`).animate({ opacity: 1 });
                });
    ...
    $('.sub_question').on('click', function (e) {
        ...
        var DivId = newDivSplit.join('_') + 'a';
            ...
            success: function (data) {
                ...
                for (i = 0; i < data.questionList.length; i++) {
                    ...
                    if (mainQuesType == 'MCQ') {
                        ...
                        if (correctAns == data.answerList[i].answerOption[0].answer_text) {
                        ...
                        if (correctAns == data.answerList[i].answerOption[1].answer_text) {
                        ...
                        if (correctAns == data.answerList[i].answerOption[2].answer_text) {
                        ...
                        if (correctAns == data.answerList[i].answerOption[3].answer_text) {
                    ...
                    $(newDivs).append(`
							<div class="question_number">Question ${newDivId}${alp[i]}<span class="pull-right question_category">${substrandList}</span>
							<br>
                            <span class="question_strategy" style="margin-right: 0px;">${categoryList}</span>
                            <br>
                            <span class="pull-left question_difficulty" style="margin-right: 0px;">(` + data.questionList[i].difficulty + ` Marks)</span>
							</div>
                ...
                // 				$('.remove_sub_question').on('click', function() {
                // 					$(this).parent().remove();
    ...
    $(document).on('click', '.remove_sub_question', function (e) {
        ...
        var parent_que_id = $(this).parent().parent().children().find('.question_order').attr('data-question_id');
        var selectedParent = $(this).parent().parent().children().find('.question_order option:selected').text();
        var selectedText = selectedParent.slice(0, -1);
        $(this).parent().remove();
    ...
    $(document).on('click', '.edit_question', function () {
        ...
        $('#generate_level').val($(this).data('level_id'));
        ...
        //loadQuestion('', '', level, subject, queType);
...
function getHeaderQuestion(id) {
    $.ajax({
        url: base_url + 'ExamMode/getHeaderEdit/' + id,
        type: 'GET',
        dataType: 'json',
        success: function (res) {
            $('.worksheet_lvl').html(level_name + ' ' + res.reqSubject);
            $('.worksheet_substr').html(res.reqSubstrand + ' / ' + res.reqTopic);
            $('.worksheet_strategy').html(res.reqStrategy);
        }
    });
}
...
function ajax_more_questions(page) {
    ...
    var difficulties = $("#gen_difficulties_form").val();
    $.ajax({
        url: base_url + 'ExamMode/getMoreQuestions/' + page,
        type: 'GET',
        dataType: 'text', // added data type
        data: {
            id: $('#generate_id').val(),
            level: $('#generate_level').val(),
            subject: $('#generate_subject').val(),
            queType: $('#generate_quetype').val(),
            generate: generate,
            substrand: $('#gen_substrand').val(),
            topic: $('#gen_topic').val(),
            heuristic: $('#gen_heuristic').val(),
            strategy: $('#gen_strategy').val(),
            que_type: $('input[name=gen_que_type]:checked').val(),
...
function update_selected_questions(question_id) {
    var que_type = $('input[name="gen_que_type"]:checked').val();
    ...
    $.ajax({
        type: 'POST',
        url: base_url + 'ExamMode/updateQuestion',
        data: {
            id: $('#generate_id').val(),
...
$('#outputPDF').on('submit', function (e) {
    ...
    $('.question_text').each(function () {
        ...
        // this removes the answer from each question
        qns = $(this).html();
