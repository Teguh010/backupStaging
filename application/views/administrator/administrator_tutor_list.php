...
<div class="section">
	<div class="container">
		<div class="row">
			...
			<form action="<?php echo base_url(); ?>administrator/add_branch_tutor" method="post">
				<div class="input-group" style="width: 100%;">
					<input type="text" placeholder="Add Tutor Email" name="add_branch_tutor" style="width:100%; padding:5px;">
											...
											foreach($users as $temp_user){
												...
												if($temp_user->status == '1'){
													echo '<label class="switch" for="'.$temp_user->user_id.'">
																<input type="checkbox" id="'.$temp_user->user_id.'" data-id="'.$temp_user->user_id.'" data-status="'.$temp_user->status.'" class="setActiveTutor">
																<span class="slider round"></span>
															</label>';
												} else {
													echo '<label class="switch" for="'.$temp_user->user_id.'">
																<input type="checkbox" id="'.$temp_user->user_id.'" data-id="'.$temp_user->user_id.'" data-status="'.$temp_user->status.'" class="setActiveTutor" checked>
																<span class="slider round"></span>
															</label>';
												}
													...
													echo '<a class="btn btn-warning btn-no-margin-top" title="Edit Profile" onclick="editProfile('.$temp_user->user_id.')"><i class="fa fa-edit"></i></a>
															...
															<a href="#" class="btn btn-no-margin-top showTutorClassBtn" title="Expand/Collapse"><i class="fa fa-caret-down"></i></a>
														</td>';
												echo '</tr>';
												$i++;
											}
...
<script type="text/javascript">
	...
	function editProfile(tutorId) {
		window.location.href = 'tutor_profile/' + tutorId ;
	}

	function editPassword(tutorId) {
		window.location.href = 'change_password/' + tutorId ;
	}

	$(document).ready(function () {

		$('.setActiveTutor').click(function () {
			...
			$.ajax({
				type: 'POST',
				url: base_url + 'Administrator/setActiveTutor',
				data: {
					user_id: user_id,
					active, active
				},
